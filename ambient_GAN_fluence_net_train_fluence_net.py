...
vali_inputs =train_images[500:600,:,:,:]
vali_truths = ground_truths[500:600,:,:,:]
train_images = train_images[:500,:,:,:]
ground_truths_train_data = ground_truths[:500,:,:,:]
...
train_truth = (
    tf.data.Dataset.from_tensor_slices(ground_truths_train_data)
    #.batch(batch_size, drop_remainder=True)
    .batch(BATCH_SIZE)
)
...
class WGAN(keras.Model):
    ...
    def train_step(self, batch_data):
        ...
        real_images,ground_truths_train = batch_data
        with tf.GradientTape() as tape:
            output_fluence = self.generator_end(real_images, training=True)

            # Calculate the discriminator loss using the fake and real image logits
            f_cost = self.f_loss_fn(gen_fluence_out=output_fluence, gen_fluence_truth=ground_truths_train)
            # Calculate the gradient penalty
            # Add the gradient penalty to the original discriminator loss
            f_loss = f_cost

            # Get the gradients w.r.t the discriminator loss
        f_gradient = tape.gradient(f_loss, self.generator_end.trainable_variables)
            # Update the weights of the discriminator using the discriminator optimizer
        self.f_optimizer.apply_gradients(
            zip(f_gradient, self.generator_end.trainable_variables)
            )

        return {"f_loss": f_loss}
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        fluence_prediction = self.model.generator_end(vali_inputs, training=False).numpy()#[0].numpy()
        vali_loss = fluence_loss(fluence_prediction,vali_truths)
        vali_loss.numpy()
        print(vali_loss.numpy())
        filename = "predictions2/vali_loss_%s.mat" % epoch
        scipy.io.savemat(filename,{'vali_loss':vali_loss.numpy()})
        
        filename = "predictions2/vali_out_%s.mat" % epoch
        scipy.io.savemat(filename,{'vali_out':fluence_prediction, 'vali_true':vali_truths.numpy()})
        
        #fluence_images = self.model.generator_end(vali_inputs)
...
checkpoint_filepath = "./model_checkpoints/checkpoints.{epoch:03d}"
model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
    filepath=checkpoint_filepath
)

# Start training the model.
history_callback=wgan.fit(tf.data.Dataset.zip((train_input, train_truth)), batch_size=BATCH_SIZE, epochs=epochs, callbacks=[cbk,model_checkpoint_callback])

f_loss_hist = history_callback.history["f_loss"]
"""Display the last generated images:

"""
scipy.io.savemat('f_loss_hist.mat', {'f_loss_hist':f_loss_hist})
